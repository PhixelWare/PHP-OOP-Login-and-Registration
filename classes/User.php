...
class User {
		...
		if (!$username && !$password && $this->exist()) {
			...
			if ($user) {
				if ($this->data()->password === Hash::make($password, $this->data()->salt)) {
					...
					if ($remember) {
						$hash = Hash::unique();
						$hashCheck = $this->_db->get('tblusers_session', array('user_id', '=', $this->data()->id));
						
						if (!$hashCheck->count()) {
							$this->_db->insert('tblusers_session', array(
	...
	public function hasPermission($key) {
		$group = $this->_db->get('tblgroups', array('id', '=', $this->data()->groups));
	...
	public function logout() {
		...
		$this->_db->delete('tblusers_session', array('user_id', '=', $this->data()->id));
